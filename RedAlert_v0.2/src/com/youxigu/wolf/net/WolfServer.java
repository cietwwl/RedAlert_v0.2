...
/**
 * 提供最方便使用的接口： 1 监听的端口号； 2 多少个线程处理消息； 3 处理消息的逻辑； 4 协议实现 5 启动服务器； 6 关闭服务器
 * 
 * 
 * 
 * 
...
public class WolfServer {
	...
	public void setAdminUser(String adminUser) {
		NodeSessionMgr.setAdminUser(adminUser);
	}

	public void setAdminPwd(String adminPwd) {
		NodeSessionMgr.setAdminPwd(adminPwd);
	}

	public void setAllowIps(String ipList) {
		NodeSessionMgr.setAllowIps(ipList);
	}
	...
	public void init() {
		...
		reloadServerAddressFromConfig();

	}

	public void reloadServerAddressFromConfig() {
		// 根据配置文件修改ip配置
		if (appProp == null)
			return;
		...
		if (str != null) {
			...
			switch (serverType) {
			case NodeSessionMgr.SERVER_TYPE_NODE:
				prefix = NodeSessionMgr.SERVER_TYPE_NODE_STR;
				break;
			case NodeSessionMgr.SERVER_TYPE_MAIN:
				prefix = NodeSessionMgr.SERVER_TYPE_MAIN_STR;
				break;
			case NodeSessionMgr.SERVER_TYPE_JOB:
				prefix = NodeSessionMgr.SERVER_TYPE_JOB_STR;
				break;
			case NodeSessionMgr.SERVER_TYPE_ACTIVE:
				prefix = NodeSessionMgr.SERVER_TYPE_ACTIVE_STR;
				break;
			case NodeSessionMgr.SERVER_TYPE_ASYNCDB:
				prefix = NodeSessionMgr.SERVER_TYPE_ASYNCDB_STR;
				break;
			case NodeSessionMgr.SERVER_TYPE_OTHER:
				prefix = NodeSessionMgr.SERVER_TYPE_OTHER_STR;
				break;

			}

			if (prefix != null) {
				this.serverName = appProp.getProperty(prefix + ".name",
						this.serverName);
				this.ip = appProp.getProperty(prefix + ".ip", this.ip);

				String tmp = appProp.getProperty(prefix + ".port");
				if (tmp != null) {
					this.port = Integer.parseInt(tmp);
				}
				tmp = appProp.getProperty(prefix + ".usr");
				if (tmp != null) {
					this.setAdminUser(tmp);
				}
				tmp = appProp.getProperty(prefix + ".pwd");
				if (tmp != null) {
					this.setAdminPwd(tmp);
				}
				tmp = appProp.getProperty(prefix + ".allowIp");
				if (tmp != null) {
					this.setAllowIps(tmp);
					if (logger.isDebugEnabled()) {
						logger.debug("allowIp:{}", tmp);
					}
				}
	...
	public void start() {
		...
		// 添加心跳过滤器,TODO:这个心跳加到线程池前好呢，还是加到线程池后好呢，纠结
		// acceptor.getSessionConfig().setReaderIdleTime(readIdleTimeOut);
		KeepAliveFilter filter = new KeepAliveFilter(
				new KeepAliveMessageFactoryProtoImpl(), IdleStatus.BOTH_IDLE,
				keepAliveHandler == null ? new RequestTimeoutCloseProtoHandler()
						: keepAliveHandler, keepAliveRequestInterval <= 0 ? 600
						: keepAliveRequestInterval, 30);
