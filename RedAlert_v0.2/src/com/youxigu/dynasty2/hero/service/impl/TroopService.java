...
public class TroopService implements ITroopService, IEventListener {
	...
	@Override
	public void doCheakIdleTroopStatus(long userId, long troopId,
			TroopIdle status, Timestamp freeDttm) {
		this.lockTroop(userId);
		Troop troop = this.getTroopById(userId, troopId);
		if (troop == null
				|| troop.getStatus() == TroopIdle.STATUS_OUT.getIndex()) {
			throw new BaseException("军团不可用");
		}
