...
public class MapService implements IMapService {
	...
	private Map<Integer, List<State>> countryStates = new HashMap<Integer, List<State>>();//每个国家绑定的所有区块，区块按照id从小到大排序
	/**记录每个国家创建角色的人数**/
	...
	/**记录已经占领的关键资源点信息**/
	private Map<Integer, OpenState> occupiedMapcells = new ConcurrentHashMap<Integer, OpenState>();
	...
	private void loadConf() {
		...
		if (countrys != null && countrys.size() > 0) {
			...
			for (Country country : countrys) {
				...
				if (states != null && states.size() > 0) {
					for (State state : states) {
						...
						List<State> ls = countryStates.get(state.getCountryId());
						if(ls==null){
							ls = new ArrayList<State>();
							countryStates.put(state.getCountryId(), ls);
						}
						ls.add(state);
					...
					//排序
					for(Entry<Integer, List<State>> en :countryStates.entrySet()){
						List<State> ls = en.getValue();
						java.util.Collections.sort(ls,new Comparator<State>() {
							@Override
			...
			//关联区域开放关系
			for(Entry<Integer, State> en : stateMaps.entrySet()){
				State st = en.getValue();
				st.setNextState(stateMaps.get(st.getNextStateId()));
			}

		}

		// 加载大地图怪配置
		this.initNPCAttackConf();
	}

	/**
	 * 加载大地图怪物配置
	 */
	private void initNPCAttackConf() {
		List<NPCAttackConf> confs = mapDao.getNPCAttackConfs();
		npcAttackConfs = new HashMap<Integer, NPCAttackConf>(confs.size());
		for (NPCAttackConf conf : confs) {
			npcAttackConfs.put(conf.getId(), conf);
		}

		for (NPCAttackConf conf : confs) {
			if (conf.getNpcId() != 0) {
				NPCAttackConf next = npcAttackConfs.get(conf.getNextId());
				if (next != null) {
					conf.setNext(next);
					next.setPrev(conf);
				}
		...
		log.info("加载大地图，野怪，个人，联盟，国家 资源点防守怪的定义完毕");
	}
	...
	@Override
	public NPCAttackConf getNPCAttackConf(int id) {
		// TODO Auto-generated method stub
		return npcAttackConfs.get(id);
	}
	...
	@Override
	public void lockMapCells(List<Integer> cellIds)
			throws InterruptedException {
		if (!ThreadLocalMapCellCache.isInTrans()) {
			throw new BaseException("只有在事务中才能锁MapCell");
		}
		...
		if (cellIds != null && cellIds.size() > 0) {
			...
			for (int cellId : cellIds) {
				MapCell cell = ALL_CELLS.get(cellId);
				if (cell.getLock().tryLock(LOCK_WAIT_TIME, TimeUnit.SECONDS)) {
    ...
    public List<Integer> getAllStateIds(){
        return new ArrayList<Integer>(stateMaps.keySet());
    }
     ...
     * @param stateCache
     */
    public void prepareStateOpen(StateCache stateCache){
    	List<OpenState> sts = mapDao.getAllOpenStatesById(stateCache.getStateId());
    	Map<Integer, OpenState> om = new HashMap<Integer, OpenState>();
    	for(OpenState s : sts){
    		om.put(s.getMapCellId(), s);
    	}
    	occupiedMapcells.putAll(om);
    	
    	State st = stateCache.getState();
    	Set<Integer> set = st.getAllMapCells();
    	boolean open = true;
    	for(int  s : set){
    		if(!occupiedMapcells.containsKey(s)){
    			open = false;
    			break;
    		}
    	}
	...
	@Override
	public void doUpdateOpenState(OpenState st) {
		if(st == null){
			return;
		}
		if(occupiedMapcells.containsKey(st.getMapCellId())){
			return;
		}
		mapDao.insertOpenState(st);
	}
	
	@Override
	public Set<Integer> getAllOpenStates(int countryId) {
		if(!countryMaps.containsKey(countryId)){
			return null;
		}
		Set<Integer> result = new HashSet<Integer>();
		//获取这个国家下面绑定的所有区域
		List<State> ls = countryStates.get(countryId);
		for(State st : ls){
			StateCache ct = STATE_CELLS.get(st.getStateId());
			if(ct == null){
				break;
			}
			if(!ct.isOpen()){
	...
	@Override
	public Point doRandomCity(int stateId) {
		// TODO Auto-generated method stub
		return null;
	}
}
