...
import com.youxigu.dynasty2.core.flex.amf.OpenPlatformTransFilter;
...
public class ProtobufWolfService implements IWolfService, IInitListener, ApplicationContextAware {
	...
	public static Logger log = LoggerFactory.getLogger(ProtobufWolfService.class);

	// /测试用的同步调用方式
	public static final String SYNC_KEY = "syncstat_";
	...
	public void setOpenPlatformTransFilter(OpenPlatformTransFilter openPlatformTransFilter) {
		...
		if (actionDefines != null) {
			...
			for (ActionDefine ad : actionDefines) {
				...
				if (ad.getSubActionIds() == null) {
					Map<Integer, Object> discardEventTypeMaps = ad.getDiscardEventTypeMaps();
	...
	public Object handleMessage(Response response, Object message) {
		if (message instanceof com.google.protobuf.Message) {
			...
			try {
				int cmd_ = 0;
				Field protoHeadF = null;
				protoHeadF = params.getClass().getDeclaredField("cmd_");
				if(protoHeadF != null) {
					protoHeadF.setAccessible(true);
					cmd_ = protoHeadF.getInt(params);
				}
				
				ActionDefine ad = actions.get(cmd_);
				if(ad==null){
					throw new BaseException("错误的命令号,找不到处理器:" + cmd_);
				}
				return _handleMessage(response, params, ad);
	...
	public Object _handleMessage(Response response, Message message, ActionDefine ad) {
		Message retu = null;
		long now = System.currentTimeMillis(); // 开始时间

		// //同步调用标志
		CommonHead.SyncStat syncstat = null;
		try {
			Field syncstatF = message.getClass().getDeclaredField(SYNC_KEY);
			if (syncstatF != null) {
				syncstatF.setAccessible(true);
				syncstat = (CommonHead.SyncStat) syncstatF.get(message);
				if (syncstat.getStat() == SyncStat.SYNC_STATUS_RESPONSE) {
					ResultMgr.requestCompleted(response.getSession(), syncstat.getId(), message);
					return Boolean.TRUE;
				} else if(syncstat.getStat() == SyncStat.SYNC_STATUS_REQUEST){
					CommonHead.SyncStat.Builder br2 = CommonHead.SyncStat.newBuilder();
					br2.setId(syncstat.getId());
					br2.setStat(SyncStat.SYNC_STATUS_RESPONSE);
					syncstat = br2.build();
					syncstatF.set(message, syncstat);
		...
		try {
			...
			if (log.isDebugEnabled()) {
				if (session != null) {
					log.debug("account:{},receive request:{}", session.getAccId(), message);
				} else {
...
*/
			if (session != null) {
				...
				if (forbidTime != null) {
					if (forbidTime > now) {
						...
						badRequest.setErrCode(-9022);
						badRequest.setErr("高频请求帐号，正在封停期间内，禁止登录,服务器关闭连接");
						badRequest.setRequestCmd(cmd);
						IoSession iosession = response.getSession();
						iosession.write(badRequest);
			...
			if (ad.isPwdcheak() && !checkPwd(session)) {
				ResponseHead.Builder badRequest = ResponseHead.newBuilder();
				badRequest.setCmd(IAMF3Action.CMD_DEFAULT);
				badRequest.setErrCode(IAMF3Action.CMD_PASSWORD);
				badRequest.setErr("您已设置了二级密码，无法使用此功能!");
				badRequest.setRequestCmd(cmd);
				IoSession iosession = response.getSession();
			...
			long userId = 0;
			if (ad.getFilterEventTypeMaps() != null || ad.getFilterChannelMaps() != null) {
				if (session != null) {
					userId = session.getUserId();
					MessageFilter.setUserId(userId);
					MessageFilter.addFilterEventTypes(ad.getFilterEventTypeMaps());
			...
			if (retu_ != null && retu_.getClass() != Void.class) {
				if (retu_ instanceof Message) {
//					System.out.println(retu_.getClass().getName());
//					try {
//						Field protoHeadF_ = retu_.getClass().getDeclaredField("protoHead_");
//						protoHeadF_.setAccessible(true);
//						ProtoHead protoHead = (ProtoHead) protoHeadF_.get(retu_);
//						protoHead.setErr("");
//						protoHeadBr.setErrCode(0);
//						protoHeadBr.setRequestCmd(cmd);
//						protoHeadBr.setCmd(protoHead.getCmd());
//						if(!protoHead.hasRequestCmd()) {
//							ProtoHead.Builder protoHeadBr = protoHead.newBuilder();
//							
//							protoHeadF_.set(retu_, protoHeadBr.build());
//						}
//					} catch (Exception e) {
//						Field requestCmdF_ = retu_.getClass().getDeclaredField("requestCmd_");
//						requestCmdF_.setAccessible(true);
//						requestCmdF_.setInt(retu_, cmd);
//						
//						Field errCodeF_ = retu_.getClass().getDeclaredField("errCode_");
//						errCodeF_.setAccessible(true);
//						errCodeF_.setInt(retu_, 0);
//						
//						Field errF_ = retu_.getClass().getDeclaredField("err_");
//						errF_.setAccessible(true);
//						errF_.set(retu_, "");
//					}
					
					if (hasFilter) {
						processFilterMessage(userId, ad,retu);
					}

//					if (syncstat != null) {
//						Field retuSyncstatF = retu_.getClass().getDeclaredField(SYNC_KEY);
//						retuSyncstatF.setAccessible(true);
//						retuSyncstatF.set(retu_, syncstat);// 默认没有异常
//					}
...
//				Map<String, Object> payExceptionMap = new HashMap<String, Object>(2);
//				payExceptionMap.put(IAMF3Action.ACTION_CMD_KEY, IAMF3Action.CMD_PAY_ERROR);
//				payExceptionMap.put(IAMF3Action.ACTION_ERROR_CODE_KEY, 0);
//				response.getSession().write(payExceptionMap);
//			}
//
//			try {
//				errCode_.setInt(message, errCode);
//				err_.set(message, errDesc);
//			} catch (Exception e2) {
//
//			}
...
//			}
			...
			// /发送给开放平台协议上报接口
//			if (session != null) {
//				protoReportService.report(ad, session, hasException, null);
//
//			} else {
//				session = (UserSession) response.get(UserSession.KEY_USERSESSION);
//				if (session != null) {
...
//			}
	...
	private void processFilterMessage(long userId, ActionDefine ad, Message retuMap) {
				...
				while (lit.hasNext()) {
					...
					if (msg instanceof EventMessage) {
						...
						if (ad.isDiscardEvent(eventType)) {
							...
							if (key != null) {
								try {
									Field obj_ = retuMap.getClass().getDeclaredField(key + "_");
									if (obj_ != null) {
										obj_.setAccessible(true);
										obj_.set(retuMap, e.getParams());// 默认没有异常
									}
								} catch (Exception e2) {
									throw new BaseException("proto结构中未包含过滤的消息类型");
								}
...
//					retuMap.put("msg", msgs);
	 ...
	 * @param response
	 */
	private UserSession validateAndRegister(Response response, Message message) {
		IoSession ioSession = response.getSession();
		UserSession us = (UserSession) ioSession.getAttribute(UserSession.KEY_USERSESSION);
		...
		String sid = null;
		try {
			Field sid_ = message.getClass().getDeclaredField("sid_");
			if (sid_ != null) {
				sid_.setAccessible(true);
				
				//TODO 此处jdk1.6版本下是ok，jdk1.7下无法拿到string
//				sid = (sid_.get(message)).toString("utf-8");
				Object obj = sid_.get(message);
				if(obj instanceof ByteString){
					sid = ((ByteString)obj).toStringUtf8();
				}else{
					sid = obj.toString();
				}
			}
...
//			e.printStackTrace();
		...
		if (us == null) {
			synchronized (ioSession) {
				us = (UserSession) ioSession.getAttribute(UserSession.KEY_USERSESSION);
				if (us == null) {
					if (sid == null) {
						if (log.isDebugEnabled()) {
							log.debug("收到没有sid的请求：来源：{},参数:{}", ioSession.getRemoteAddress(), message);
						}
						throw new BaseException("非法请求，请先登陆或者提供sid:");
					}
					if (mainClient != null) {
						us = mainClient.sendTask(UserSession.class, "wolf_onlineUserService", "getUserSession", sid);
					}

					if (us == null) {
						if (log.isDebugEnabled()) {
							log.debug("收到没有登陆的请求：来源：{},参数:{}", ioSession.getRemoteAddress(), message);
						}
	...
	private boolean checkAddicted(UserSession us) {
		if (accountService == null) {
			accountService = (IAccountService) ServiceLocator.getSpringBean("accountService");
		}
	...
	private boolean checkRequestNumLimit(Response response, Message message, int cmd, UserSession userSession) {
		IoSession ioSession = response.getSession();
		synchronized (ioSession) {
			...
			if (requestCount > startRequestNum && requestCount % maxRequestPerSecond == 0) {
				// 检查时间
				Long lastTime = (Long) ioSession.getAttribute(SESSION_LAST_REQUEST_TIME);
				if (lastTime == null) {
					...
					if (now - lastTime <= 60000) {
						...
						log.error("请求频率过快，关闭连接:{},{}", session.getLocalAddress(), userSession == null ? ""
								: userSession.getAccName());

						if (userSession != null) {
							forbidOpenIds.put(userSession.getOpenid(), now + forbidTimes);
						}
