...
import com.youxigu.dynasty2.user.proto.UserMsg.Request10048Define;
...
import com.youxigu.dynasty2.user.proto.UserMsg.Response10048Define;
...
public class UserAction extends BaseAction {
	 ...
	 * @return
	 */
	private boolean canCreateRoleByCountry(int countryId){
		//判断这个国家是否还可以创建角色
		int smax = commonService.getSysParaIntValue(AppConstants.COUNTRY_CREATE_USER_NUM, 2000);
		int huser = mapService.getAllUserCount();
		if(huser> smax){
			//判断占比
			int mp = commonService.getSysParaIntValue(AppConstants.COUNTRY_CREATE_USER_NUM_PERCENTAGE, 40);
			int num = mapService.getCountryUser(countryId);
			if(((num/huser)*(100))>mp){
				return false;
			}
		}
	...
	public Object createUser(Object params, Response context) {
		...
		if(!canCreateRoleByCountry(countryId)){
			throw new BaseException("当前国家人数已满,请选择其他国家注册");
		}
		...
		mapService.addCountryUser(countryId);
	 ...
	 * @return
	 */
	public Object canCreateRole(Object obj, Response context) {
		Request10048Define req = (Request10048Define)obj;
		Set<Integer> st = mapService.getCountryIds();
		Response10048Define.Builder res = Response10048Define.newBuilder();
		for(Integer i : st){
			boolean b = canCreateRoleByCountry(i);
			res.addCountrys(i);
			res.addCreateRole(b);
		}
		res.setResponseHead(super.getResponseHead(req.getCmd()));
