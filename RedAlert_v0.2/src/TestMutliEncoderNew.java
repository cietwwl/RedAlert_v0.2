...
import com.youxigu.wolf.net.codec.IoBufferOutputStream;
 ...
 * 
 */
public class TestMutliEncoderNew extends ProtocolEncoderAdapter {
	private static Logger log = LoggerFactory.getLogger(TestMutliEncoderNew.class);
		...
		if (message instanceof String) {// flash 安全沙箱或者TGW头不encode,
			...
			if (byteLen > 1024) {
				...
				if (log.isWarnEnabled()) {
					log.warn("消息体太长，被压缩,from {} bytes to {} bytes.", oldLen,
							byteLen);
				}
			...
			if(byteLen>1024*1024){
				buffer.putInt(byteLen +1024*1024*512);
			}else{
	...
	private void compress(IoSession session, byte[] inputs, int offset,
		...
		try {
			...
			// while (!deflater.finished()) {
			// int value = deflater.deflate(buf, 0, cachesize);
			// buffer.put(buf, 0, value);
			// // total = total + value;
			// }
		} finally {
			deflater.end();
	...
	// try {
	// MutliEncoderNew encoder = new MutliEncoderNew();
	...
	// IoBuffer buffer = IoBuffer.allocate(64, false);
	// buffer.position(HEAD_LEN);
	// encoder.getAmf3Bytes(buffer, map);
	// int position = buffer.position();
	// int byteLen = position - HEAD_LEN;
