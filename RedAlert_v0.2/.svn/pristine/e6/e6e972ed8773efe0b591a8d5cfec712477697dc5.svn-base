...
/**
 * copy from quartz.CronExpression
 * 
 * 表达式意义
 * 
 * "0 0 12 * * ?" 每天中午12点触发
 ...
 * 
 * "0 15 10 ? * MON-FRI" 周一至周五的上午10:15触发 * * 7-23 ? * MON-SAT 周一至周六的7点-23点
...
public class CronExpression implements Serializable, Cloneable {
	...
	public boolean isSatisfiedBy(long millis) {
		Calendar testDateCal = Calendar.getInstance(getTimeZone());
		testDateCal.setTimeInMillis(millis);
		testDateCal.set(Calendar.MILLISECOND, 0);
		Date originalDate = testDateCal.getTime();
	...
	public Date getNextValidTimeAfter(long time) {
		return getTimeAfter(time);
	}

	public Date getNextInvalidTimeAfterByMinute(Date date) {
		long time = date.getTime() + 60000;
		while (this.isSatisfiedBy(time)) {
			time = time + 60000;
		}
	...
	protected Date getTimeAfter(Date afterTime) {
		return getTimeAfter(afterTime.getTime());
	}

	protected Date getTimeAfter(long after) {
		...
		// given time
		after = after + 1000;
		Date afterTime = new Date(after);
		// CronTrigger does not deal with milliseconds
		cl.setTimeInMillis(after);
