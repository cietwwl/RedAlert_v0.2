...
public class MapCell implements java.io.Serializable {
    private static final long serialVersionUID = -2631512197808409541L;

	public static final String KEY = "MapCell_";

	// castType; 城池类型 casId>0,对应的casId为玩家城池id或是npc和资源的配数id
    public static final int CAS_TYPE_UNDEFINE = 0;//未定义，初始值
	...
	private int castType;// 城池类型
	private long casId; // 城池编号(可能是玩家/野怪(NPCAttackConf.id)/个人资源点
						// 通过castleType来区别)
	private long userId;// 领地君主-个人资源地
	private long guildId;// 领地联盟-联盟资源地和国家资源地

	private int earthId;// 地表对应的配置id
	private int earthType;// 地表类型 个人、联盟，国家 资源点

	private transient State state;// 所属区
	private transient Country country;// 所属国家
	private transient Map<Long, Armyout> fromArmyouts = new HashMap<Long, Armyout>();// 从该坐标发出的出征
	private transient Map<Long, Armyout> toArmyouts = new HashMap<Long, Armyout>();// 发往该坐标的出征
	private transient Map<Long, Armyout> stayArmyouts = new HashMap<Long, Armyout>();// 停留在该坐标的出征
