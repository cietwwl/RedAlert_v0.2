 ...
 *
 */
public class NPCHeroCombatUnit extends CombatUnit {
	/**
	 * 
	 */
	private static final long serialVersionUID = 724708930002977004L;
	private transient NPCHero hero;
	private transient IEntityService entityService;

	public NPCHeroCombatUnit() {
		super();
	}

	public NPCHeroCombatUnit(NPCHero hero, int id) {
		this();
		this.id = id;
		this.name = hero.getName();
		SysHero syshero = hero.getSysHero();
		this.hero = hero;
		this.icon = syshero.getPicPath();
		this.level = hero.getLevel();
		this.unitEntId = hero.getSysHeroId();
		this.armyEntId = hero.getArmy().getEntId();
		this.initHp = hero._getInitHp();
		this.curHp = this.initHp;
		this.initMorale = hero._getMorale();
		this.curMorale = this.initMorale;
		this.unitType = CombatConstants.COMBATUNIT_TYPE_NPCHERO;
		...
		List<Integer> list = syshero.getSkills();
	...
	private Army _getArmy() {
		return hero.getArmy();
	}

	public NPCHero _getHero() {
		return hero;
	}
