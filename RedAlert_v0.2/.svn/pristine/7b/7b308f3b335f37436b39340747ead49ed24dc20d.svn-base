...
/*
 * 
 *  copy自mina的 OrderedThreadPoolExecutor，
 *  主要是为了防止一个session中的请求太多,
 *  本来想在IoEventQueueHandler中处理，
 *  可是SessionTasksQueue是私有类，
 *  直接在代码里面判断长度了
 *  也可以加个方法，得到SessionTasksQueue的长度，然后在IoEventQueueHandler中处理
 *  
 *  
...
public class MyOrderedThreadPoolExecutor extends ThreadPoolExecutor {
	...
	static Logger LOGGER = LoggerFactory
			.getLogger(OrderedThreadPoolExecutor.class);
	 ...
	 * 一个session的queueSize
	 */
	private volatile int maxQueueSize=Integer.MAX_VALUE;
	 ...
	 */
	public MyOrderedThreadPoolExecutor() {
		this(DEFAULT_INITIAL_THREAD_POOL_SIZE, DEFAULT_MAX_THREAD_POOL,
				DEFAULT_KEEP_ALIVE, TimeUnit.SECONDS, Executors
						.defaultThreadFactory(), null, Integer.MAX_VALUE);
	}
	 ...
	 */
	public MyOrderedThreadPoolExecutor(int maximumPoolSize) {
	...
	public MyOrderedThreadPoolExecutor(int corePoolSize, int maximumPoolSize,
		...
		this(corePoolSize, maximumPoolSize, keepAliveTime, unit, Executors
				.defaultThreadFactory(), eventQueueHandler, Integer.MAX_VALUE);
	}
	...
	private SessionTasksQueue getSessionTasksQueue(IoSession session) {
		SessionTasksQueue queue = (SessionTasksQueue) session
				.getAttribute(TASKS_QUEUE);

		if (queue == null) {
			queue = new SessionTasksQueue();
			SessionTasksQueue oldQueue = (SessionTasksQueue) session
					.setAttributeIfAbsent(TASKS_QUEUE, queue);
	...
	public List<Runnable> shutdownNow() {
		...
		while ((session = waitingSessions.poll()) != null) {
			...
			SessionTasksQueue sessionTasksQueue = (SessionTasksQueue) session
					.getAttribute(TASKS_QUEUE);
	...
	private void print(Queue<Runnable> queue, IoEvent event) {
		...
		sb.append("Adding event ").append(event.getType()).append(
				" to session ").append(event.getSession().getId());
		boolean first = true;
		sb.append("\nQueue : [");
	...
	public void execute(Runnable task) {
		...
		if (tasksQueue.size() > this.maxQueueSize) {
			LOGGER.error("session:{}的请求队列大于{},拒绝", session.getRemoteAddress(),
					this.maxQueueSize);
			rejectTask(task);
			return;
		}
//		else{
//			LOGGER.error("session:{}的请求队列={}", session.getRemoteAddress(),
//					tasksQueue.size());
//			
//		}
	...
	private class Worker implements Runnable {
		...
		public void run() {
			...
			try {
				...
				synchronized (workers) {
					workers.remove(this);
					MyOrderedThreadPoolExecutor.this.completedTaskCount += completedTaskCount;
					workers.notifyAll();
				}
			}
		}
		...
		private IoSession fetchSession() {
			...
			for (;;) {
				try {
					...
					try {
						session = waitingSessions.poll(waitTime,
		...
		private void runTasks(SessionTasksQueue sessionTasksQueue) {
			for (;;) {
				...
				eventQueueHandler.polled(MyOrderedThreadPoolExecutor.this,
						(IoEvent) task);
