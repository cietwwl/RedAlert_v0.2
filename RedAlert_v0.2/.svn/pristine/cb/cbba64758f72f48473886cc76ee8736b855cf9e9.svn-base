...
public class SysHero extends Entity {
	...
	private transient SysHeroCountry country;
	...
	public void check(SysHeroCountry sysHeroCountry) {
		if (sysHeroCountry == null) {
			throw new BaseException("武将所属的国家id未配置[sysHeroCountry]"
					+ getCountryId());
		}
		...
		if (getHeroCardId() <= 0) {// 如果是创建角色给的武将 策划说默认不需要配置这个
	...
	public SysHeroCountry getCountry() {
		return country;
	}
}
